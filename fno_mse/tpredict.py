...
if __name__ == "__main__":
    ...
    n_samples = 22
    split = [0.6, 0.2, 0.2]
    image_ids = np.random.randint(low=0, high=101, size=(n_samples,))
    ...
    quality = np.empty((2*len(test_ids), 20), dtype=np.float64)


    for i, batch in enumerate(predictions):
        np.save(npy_path / f"Sample{test_ids[i]:04}_prediction", batch["jhat"][-1][0, :, :, :].squeeze().cpu().numpy())
        np.save(npy_path / f"Sample{test_ids[i]:04}_truth", batch["j"][-1][0, :, :, :].squeeze().cpu().numpy())
        geom_file = Path(f"/scratch/08780/cedar996/lbfoam/fno/lbfoam_sims/135_{test_ids[i]:04}_1280_550/fracture.raw")
        data_truth = batch["j"][-1][0, :, :, :].squeeze().cpu().numpy()#.reshape((550,1280,20))
        data_pred = batch["jhat"][-1][0, :, :, :].squeeze().cpu().numpy()#.reshape((550,1280,20))
        ...
        plt.legend()
        plt.ylim(0,1)
        plt.savefig(png_path / f'Cquality_Sample{test_ids[i]:04}.png', dpi=300)
    ...
    # for m in range(0, 2*len(test_ids),2):
    #     plt.figure()
    #     plt.plot(quality[m, :],  'k--', label='Ground Truth')
    #     plt.plot(quality[m+1, :], 'r--', label='Prediction')
    #     plt.xlabel('Time step')
    #     plt.ylabel('Quality')
    #     plt.legend()
    #     plt.savefig(png_path/f'Cquality_Sample{test_ids[i]:04}.png', dpi=300)
...
#    quality_df.columns.name = list(np.arange(20))
