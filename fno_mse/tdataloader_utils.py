...
class NumpyDataset(Dataset):
    def __init__(self, image_ids, data_dir):
    ...
    def __getitem__(self, idx):
        try:
            ...
            input_field = np.fromfile(self.data_dir / f"135_{self.image_ids[idx]:04}_1280_550"/"input_binary10.raw", dtype=np.uint8).reshape((T_in, 550,1280))
            ...
            input_field = input_field.reshape(550, 1280, 1, T_in).repeat([1, 1, T_out, 1])
            # Current Density Fields
            output_field = np.fromfile(self.data_dir / f"135_{self.image_ids[idx]:04}_1280_550"/"target_binary10.raw", dtype=np.uint8).reshape((T_out, 550, 1280))
...
def get_dataloader(image_ids, data_path, split=[0.6, 0.2, 0.2], batch=1, num_workers=2, seed=1261613, **kwargs):

    dataset = NumpyDataset(image_ids=image_ids, data_dir=data_path)
